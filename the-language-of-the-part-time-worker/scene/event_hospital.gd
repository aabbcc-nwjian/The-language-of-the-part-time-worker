...
var scene_paths = {
	"hospital": "res://scene/yiyuan.tscn"
}
...
func load_event_data() -> void:
	var file := FileAccess.open("res://Events/hospital.json", FileAccess.READ)
	if file:
		...
		print("无法打开事件文件：res://Events/hospital.json")
	file.close()
...
func _on_option_selected(effect: Dictionary) -> void:
	...
	get_tree().change_scene_to_file(scene_paths["hospital"])
